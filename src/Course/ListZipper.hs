...
instance Functor MaybeListZipper where
  -- f <$> (MLZ optLZ) = MLZ $ (f <$>) <$> optLZ
  f <$> (MLZ (Full l)) = isZ $ f <$> l
  f <$> _ = isNotZ
...
toList ::
  ListZipper a ->
  List a
toList lza = lefts lza ++ focus lza :. rights lza
...
fromList ::
  ...
  MaybeListZipper a
fromList Nil = isNotZ
fromList (head :. rest) = isZ $ ListZipper Nil head rest
...
withFocus ::
  ...
  ListZipper a ->
  ListZipper a
withFocus f lz = ListZipper (lefts lz) (f $ focus lz) (rights lz)
...
hasLeft ::
  ListZipper a ->
  Bool
hasLeft (ListZipper left _ _) = (not . isEmpty) left
...
hasRight ::
  ListZipper a ->
  Bool
hasRight (ListZipper _ _ right) = (not . isEmpty) right
...
findLeft ::
  ...
  ListZipper a ->
  MaybeListZipper a
findLeft f (ListZipper Nil _ _) = isNotZ
findLeft f (ListZipper left focus right) = case break f left of
  (_, Nil) -> isNotZ
  (broken, newFocus :. newLeft) -> isZ $ ListZipper newLeft newFocus newRight
    where
      newRight = reverse broken ++ focus :. right
...
findRight ::
  ...
  ListZipper a ->
  MaybeListZipper a
findRight f (ListZipper Nil _ _) = isNotZ
findRight f (ListZipper left focus right) = case break f right of
  (_, Nil) -> isNotZ
  (broken, newFocus :. newRight) -> isZ $ ListZipper newLeft newFocus newRight
    where
      newLeft = reverse broken ++ focus :. left
...
moveLeftLoop ::
  ListZipper a ->
  ListZipper a
moveLeftLoop (ListZipper Nil focus right) = ListZipper newLeft newFocus Nil
  where
    (newFocus :. newLeft) = reverse (focus :. right)
moveLeftLoop (ListZipper left focus right) = ListZipper newLeft newFocus newRight
  where
    (newFocus :. newLeft) = left
    newRight = focus :. right
...
moveRightLoop ::
  ListZipper a ->
  ListZipper a
moveRightLoop (ListZipper left focus Nil) = ListZipper Nil newFocus newRight
  where
    (newFocus :. newRight) = reverse (focus :. left)
moveRightLoop (ListZipper left focus right) = ListZipper newLeft newFocus newRight
  where
    (newFocus :. newRight) = right
    newLeft = focus :. left
...
moveLeft (ListZipper (leftHead :. leftRest) focus right) = isZ $ ListZipper leftRest leftHead (focus :. right)
...
moveRight (ListZipper left focus (rightHead :. rightRest)) = isZ $ ListZipper (focus :. left) rightHead rightRest
...
swapLeft (ListZipper (leftHead :. leftRest) focus right) = isZ $ ListZipper (focus :. leftRest) leftHead right
...
swpRight (ListZipper left focus (rightHead :. rightRest)) = isZ $ ListZipper left rightHead (focus :. rightRest)
...
moveLeftN n lz
  ...
  | n > 0 = asMaybeZipper (moveLeftN (n - 1)) $ moveLeft lz
...
moveRightN n lz
  ...
  | n > 0 = asMaybeZipper (moveRightN (n - 1)) $ moveRight lz
